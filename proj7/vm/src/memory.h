...
/* Set size and tag in a block's header */
void memory_set_block_size_and_tag(value_t* block, tag_t tag, value_t size);

/* Checks whether the given block size can be split to store the given size */
int block_too_small_to_split(value_t size, value_t block_size);

/* Sets the given block to point to the given ptr */
