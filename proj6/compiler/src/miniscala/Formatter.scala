...
object Formatter {
  ...
  def foldDoc(docs: Seq[Document], sep: String = " ", break: Boolean = true): Document = docs match {
    case Seq() => empty
    case Seq(d) => d
    case Seq(d, ds @ _*) => d :: (if (break)
      anyToDoc(sep) :/: foldDoc(ds, sep, break)
    else
      anyToDoc(sep) :: foldDoc(ds, sep, break))
  }

  def seqToDoc[T](docs: Seq[T], toDoc: T=>Document): Document =
    foldDoc(docs map toDoc, ", ", false)
