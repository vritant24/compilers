...
class ValueInterpreter extends Interpreter with BugReporter {
  ...
  case class ValueEnv(
    imm: Map[String, Val] = Map.empty,
    mut: HashMap[String, Val] = HashMap.empty,
    outer: Env = new Env) extends Env {

      /**
       * Add a new mutable variable 'name' in the scope
       * and assign it to 'v'.
       * Return the current state.
       */
      def withVar(name: String, v: Val): ValueEnv = {
        mut(name) = v
        this
      }
      ...
      override def apply(name: String): Val = {
        if (imm.contains(name))
          imm(name)
        else if (mut.contains(name))
          mut(name)
        else
          outer(name)
      }
