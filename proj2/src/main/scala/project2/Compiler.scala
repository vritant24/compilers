...
abstract class X86Compiler extends BugReporter with Codegen {
  ...
  def transUn(op: String)(sp: Loc) = op match {
    case "+" => 
      emitln(s"movq $$0, ${regs(sp+1)}")
      emitln(s"addq ${regs(sp+1)}, ${regs(sp)}")
    case "-" =>
      emitln(s"movq $$0, ${regs(sp+1)}")
      emitln(s"subq ${regs(sp)}, ${regs(sp+1)}")
  ...
  def trans(exp: Exp, sp: Loc)(env: LocationEnv): Unit = exp match {
    ...
    case Let(x, a, b) => 
      ...
      emitln(s"movq ${regs(sp+1)}, ${regs(sp)}")
    case Ref(x) =>
      val index = env.apply(x)
      emitln(s"movq ${regs(index)}, ${regs(sp)}")
    case Cond(op, l, r) =>
      trans(l, sp)(env)
      trans(r, sp+1)(env)
    ...
    case If(cond, tBranch, eBranch) => 
      trans(cond, sp)(env)
      var lab = freshLabel("branch")
      transJumpIf(cond.op)(s"${lab}_true")
      trans(eBranch, sp)(env)
      emitln(s"jmp ${lab}_after")
      emitln(s"${lab}_true :", 0)
      trans(tBranch, sp)(env)
    ...
    case VarAssign(x, rhs) => 
      val index = env.apply(x)
      trans(rhs, sp)(env)
      emitln(s"movq ${regs(sp)}, ${regs(index)}")
    case While(cond, lBody, body) =>
