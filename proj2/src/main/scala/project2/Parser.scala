...
class Scanner(in: Reader[Char]) extends Reader[Tokens.Token] with Reporter {
  ...
  val isKeyword    = Set[String]("val", "var", "if", "while", "else")
  ...
  def isValidInt(str: String) = {
    val max = "2147483647"
    val len = str.length;
    
    if (len > max.length || str.substring(0,1) == "-") {
      false
    }
    else if (len == max.length && str < max) {
      false
    }
    else {
    true
...
class ArithParser(in: Scanner) extends Parser(in) {
  ...
  def parseExpression(min: Int): Exp = {
    var res = parseUAtom
    while (in.hasNext(isOperator)) {
      val Ident(op) = in.peek
      val newMin = prec(op)

      if (newMin >= min) {
        val (op, pos) = getOperator
        val lop = parseExpression(newMin + assoc(op))
        res = Prim(op, res, lop).withPos(pos)
      } else {
        return res  
      }
    }
...
class BranchParser(in: Scanner) extends LetParser(in) {
  ...
  override def parseSimpleExpression = in.peek match {
    case Keyword("if") =>
      ...
      accept(')')
      var tbranch = super.parseSimpleExpression
      var res = in.peek match {
        case Keyword("else") =>
          in.next()
          var ebranch = super.parseSimpleExpression
...
class VariableParser(in: Scanner) extends BranchParser(in) {
  ...
  override def parseSimpleExpression = (in.peek, in.peek1) match {
    case (Ident(x), Delim('=')) =>
      ...
      val (name, pos) = getName
      accept('=')
      var rhs = super.parseSimpleExpression
...
class LoopParser(in: Scanner) extends VariableParser(in) {
  ...
  override def parseExpression = in.peek match {
    case Keyword("while") =>
      ...
      accept(';')
      While(cond, lBody, parseExpression)
    case _ => 
