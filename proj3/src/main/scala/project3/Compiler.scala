...
abstract class X86Compiler extends BugReporter with Codegen {
  ...
  def trans(exp: Exp, sp: Loc)(env: LocationEnv): Unit = exp match {
    ...
    case LetRec(funs, body) =>
      ...
      //go to each function and emit their lines
      funs map { case f@FunDef(_, _, _, _) => trans(f, Reg(0))(env.withVals(funsLoc))}
      ...
      trans(body, Reg(0))(LocationEnv().withVals(funsLoc))
    ...
    case FunDef(fname, args, _, fbody) =>
      ...
      // TODO
      //move references to args in registers
      val idxs = List.tabulate(args.length)(i => sp + i)

      //locations of all args
      val arglocs = (args zip idxs) map {case (Arg(name, _, _), idx) => (name, idx)}

      //eval body
      trans(fbody, sp + args.length)(env.withVals(arglocs))
      emitln(s"movq ${loc(sp + args.length)}, %rax")
    ...
    case App(fun, args) =>
      ...
      // TODO
      val idxs = List.tabulate(args.length)(i => sp + i)
      (args zip idxs) map {case (arg, idx) => trans(arg, idx)(env)}
      ...
      //loop through and push everything to stack]
      val spInt:Int = sp match {
        case Reg(x) => x
      }

      var i = 0;
      while(i < spInt) {
        emitln(s"pushq ${loc(i)}")
        i = i + 1;
      }

      //move arguements to 0
      i = 0;
      while(i < spInt) {
    ...
    case ArrayDec(size, _) =>
      ...
      //move heap's loc into current reg
      emitln(s"movq heap(%rip), ${loc(sp)}")

      //eval size and store in sp+1
      trans(size, sp + 1)(env)

      //move heap's loc beyond array size
      emitln(s"movq heap(%rip), ${loc(sp+2)}")
      emitln(s"movq (${loc(sp+2)}, ${loc(sp + 1)}, 8), ${loc(sp+2)}")
      emitln(s"movq ${loc(sp+2)}, %rip")
      // val x = 8;
      // emitln(s"movq $$$x, ${loc(sp+2)}")
      // emitln(s"imulq ${loc(sp + 1)}, ${loc(sp+2)}")
      // emitln(s"addq ${loc(sp+2)}, heap(%rip)")
