...
class SyntacticSugarParser(in: Scanner) extends BaseParser(in) {
  ...
  override def parseSimpleExpression = in.peek match {
    case Keyword("if") =>
        ...
        accept(')')
        val tBranch = parseSimpleExpression
        val res = if (in.peek == Keyword("else")) {
          in.next()
          If(cond, tBranch, parseSimpleExpression).withPos(pos)
        } else {
          If(cond, tBranch, Lit(Unit)).withPos(pos)
        }
        res
...
class FunctionParser(in: Scanner) extends SyntacticSugarParser(in) {
   ...
   */
  def parseArg: Arg = {
    val (argName, pos) = getName
    accept(':')
    val argType = parseType
    Arg(argName, argType, pos)
  }
  ...
  def parseFunction: Exp = {
    in.next() //skip 'def'
    val (funName, pos) = getName
    accept('(')
    val list = parseList[Arg](parseArg, ',', tok => tok match {
      case Ident(x) => true;
      case _ => false
    })
    accept(')')
    ...
    val typList = list.map { case Arg(name, tp, _) => (name, tp) }
    var funType = FunType(typList, rtp)
    FunDef(funName, list, funType, body).withPos(pos)
  }
  ...
  def parseProgram = in.peek match {
    case Keyword("def") =>
      val list = parseList[Exp](parseFunction, ';', tok => tok match {
        case Keyword("def") => true;
        case _ => false
      })
      accept(';')
      LetRec(list, parseExpression)
    case _ => 
  ...
  def parseTight = in.peek match {
    ...
    case  _ =>
      ...
      while(in.peek == Delim('(')) {
        val pos = in.next().pos
        val list = parseList[Exp](parseSimpleExpression, ',', tok => tok match {
          case Delim(')') => false;
          case _ => true
        })
        accept(')')
        res = App(res, list).withPos(pos)
      }
      res
  }
