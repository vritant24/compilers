...
class StackInterpreter extends Interpreter with BugReporter {
  ...
  def eval(exp: Exp, sp: Loc)(env: LocationEnv): Unit = exp match {
    ...
    case LetRec(funs, body) =>
      ...
      //eval all functions
      val idxs = List.tabulate(funs.length)(i => sp + i)

      // add functions to the environment
      val funcs = (funs zip idxs) map { case (fun@FunDef(name, _, _, _), idx) => (name, idx) }

      (funs zip idxs) foreach { case (fun@FunDef(_, _, _, _), idx:Int) => eval(fun, idx)(env.withVals(funcs)) }

      eval(body, sp + funs.length)(env.withVals(funcs))
      memory(sp) = memory(sp + funs.length)
    case App(fun, args) =>  
      // //Do block get and block set
       // Evaluate the arguments
      val idxs = List.tabulate(args.length)(i => sp + i)
      (args zip idxs) map { case (arg, idx) => eval(arg, idx)(env) }

      // Evaluate the function to be called.
      eval(fun, sp + args.length)(env)
      memory(sp + args.length) match {
        case Func(fargs, fbody, fenv) =>
          eval(fbody, sp + args.length + 1)(fenv.withVals(fargs zip idxs))
